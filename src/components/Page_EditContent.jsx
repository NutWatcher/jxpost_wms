            ...
            <Form {...formItemLayout} onSubmit={this.handleSubmit} className="Step_1">
                ...
                <Form.Item label="课程分类">
                    {getFieldDecorator('class', {
                        ...
                        <Select size="large" mode="multiple" placeholder="课程分类可以多选" optionFilterProp={'name'}>
                            {
                ...
                <Form.Item>
                    ...
                    <Button size="large" className="step_button" onClick={this.props.cancel}>取消编辑</Button>
                </Form.Item>
            </Form>
        );
    }
    ...
    render () {
        return (
            <div className="Step_2">
                ...
                <Button size="large" style={{ marginLeft: '20px' }} onClick={this.props.cancel} className="step_button">取消编辑</Button>
...
class C_Page_EditContent extends React.Component {
    state = {
        current: 0,
        content: {
            id: 0,
    ...
    componentWillReceiveProps (next) {
        const classification = next.data.classificationList || [];
        let classList = [];
        classification.forEach(element => {
            classList.push(element.id);
        });
        let content = {
            id: next.data.id || 0,
            title: next.data.title || '',
            introduction: next.data.introduction || '',
            class: classList || [],
            classificationList: next.data.classificationList || [],
            content: next.data.content ? JSON.parse(next.data.content) : {
                bannerFileList: [],
                infoFileList: []
            }
        };
        this.setState({ current: 0, content: content });
    }
    fetchEditCourse = () => {
        ...
        fetch('/v1/course/edit', {
            ...
            .then((resonseData) => {
                ...
                if (resonseData.code !== 2000) {
                    message.error(`修改失败!:${resonseData.msg}` || '修改失败!');
                } else {
            ...
            .catch((error) => {
                console.log(error);
                message.error(`修改失败!:${error.toString()}` || '修改失败!');
            });
    };
    next = (now, value) => {
        if (now === 0) {
    ...
    render () {
        ...
        const steps = [
            {
                title: '课程基本信息',
                content: <WrappedStep_1 cancel={this.props.cancel}
                    title={this.state.content.title} introduction={this.state.content.introduction}
                    class={this.state.content.class} next={this.next}/>
            }, {
                title: '课程图文介绍',
                content: <Step_2 values={this.state.content.content} cancel={this.props.cancel}
                    next={this.next} pre = {this.pre}/>
            }, {
                title: '预览',
...
export default C_Page_EditContent;
